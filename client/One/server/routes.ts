...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Cache-busting direct app route that bypasses landing page
  app.get("/app", (req, res, next) => {
    // Force no-cache headers for mobile browsers
    res.set({
    ...
    console.log(`[${new Date().toISOString()}] Direct app access bypassing landing page for: ${req.get('User-Agent')}`);
    next();
  });
